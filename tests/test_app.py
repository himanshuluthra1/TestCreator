...
def test_build_unmatched_summary_preserves_comma_formatted_remaining_qty():
    ...
    unmatched_rows = left_df.copy()
    criteria = [
        {"col1": "Drug Code", "col2": "Drug Code", "datatype": "string", "operator": "eq"},
        {"col1": "Remaining Qty", "col2": "StockQty", "datatype": "number", "operator": "eq"},
    ]

    summary = build_unmatched_summary(left_df, right_df, unmatched_rows, criteria)

    assert list(summary["Drug Code"]) == ["D001"]
    assert pd.notna(summary.loc[0, "Remaining Qty"])
    assert float(summary.loc[0, "Remaining Qty"]) == 1200.0
...
def test_full_compare_flow(client):
    ...
    import app as app_module
    saved_files = os.listdir(app_module.OUTPUT_FOLDER)
    assert "order.xlsx" in saved_files
    assert "file1.xlsx" in saved_files
    assert any("_InitialOrder_" in name and name.endswith(".xlsx") for name in saved_files)
...
def test_compare_generates_stepwise_outputs_for_multiple_files(client):
    ...
    assert any(n == "compare_2_Unmatched.xlsx" for n in names)
    assert any(n == "compare_3_Order.xlsx" for n in names)
    assert any(n == "compare_3_Unmatched.xlsx" for n in names)
    ...
    # Per-step unmatched should be persisted in outputs/ as well.
    import app as app_module
    saved_files = os.listdir(app_module.OUTPUT_FOLDER)
    assert "compare_2_Unmatched.xlsx" in saved_files
    assert "compare_3_Unmatched.xlsx" in saved_files
